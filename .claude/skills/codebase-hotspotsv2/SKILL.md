...
All foundational concepts — **Entry point**, **Source**, **Sink**, **Data validation**,
and **Risky processing** — are defined in the `# Definition` section of
`.claude/skills/codebase-hotspotsv2/shared-rules.md`. Read that section before starting
the analysis.
...
| `agent-jwt-validation/` | Insecure JWT access token validation — algorithm confusion (symmetric key, `alg` read from token, `alg: none`), missing claim presence/value checks (`iss`, `scope`, `aud`, `sub`, `jti`, `exp`), expiration/not-before not enforced, forbidden header acceptance (`jku`, `x5u`, `jwk`), unrestricted `kid`, missing token size limit, missing `jti` revocation check | [secure-jwt-validation](https://raw.githubusercontent.com/righettod/code-assistant-skills-security-utils/refs/heads/main/.claude/skills/secure-jwt-validation/SKILL.md) |
| `agent-template-rendering/` | Insecure template engine usage — user-controlled input used as template source (SSTI / CWE-1336), template filename or path derived from user input (path traversal / CWE-22), unsafe engine configuration (sandbox escape), output auto-escaping disabled (XSS / CWE-79) | [secure-template-rendering](https://raw.githubusercontent.com/righettod/code-assistant-skills-security-utils/refs/heads/main/.claude/skills/secure-template-rendering/SKILL.md) |
| `agent-generic/` | Fallback for all sink types not covered by a dedicated agent above — XXE, SSRF, XSS, command injection, SQL/NoSQL/ORM/LDAP/XPath/GraphQL injection, path traversal, authentication bypass, authorization bypass / IDOR, CORS bypass, code injection, insecure deserialization, prototype pollution, weak RNG, uncontrolled resource allocation | Internal — rules defined in [agent-generic/SKILL.md#risky-processing](agent-generic/SKILL.md#risky-processing) |
