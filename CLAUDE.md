...
| `codebase-hotspots` | `/codebase-hotspots [path]` | Taint-traces each entry point to risky sinks; outputs a structured findings report saved as `Findings-YYYY-MM-DD.md` and a companion test file `SecurityFindingsTest-YYYY-MM-DD.<ext>` |
...
4. Run `/codebase-hotspots` to trace entry-point → sink paths.
